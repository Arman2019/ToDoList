...
function CreateToDoItems() {
    if (todoValue.value === "") {
        ...
        const todoItems = `<div title="Hit Double Click and Complete" ondblclick="CompletedToDoItems(this)">${todoValue.value}</div><div>
                      <img class="edit todo-controls" onclick="UpdateToDoItems(this)" src="img/icon/edit.png" />
                      <img class="delete todo-controls" onclick="DeleteToDoItems(this)" src="img/icon/trash.png" /></div></div>`;
...
function ReadToDoItems() {
    todo.forEach((element) => {
        ...
        const todoItems = `<div ${style} title="Hit Double Click and Complete" ondblclick="CompletedToDoItems(this)">${element.item}
             ${style === "" ? "" : '<img class="todo-controls" src="img/icon/check-mark.png" />'}</div>
             <div>${style === "" ? '<img class="edit todo-controls" onclick="UpdateToDoItems(this)" src="img/icon/edit.png" />' : ""}
             <img class="delete todo-controls" onclick="DeleteToDoItems(this)" src="img/icon/trash.png" /></div></div>`;
        li.innerHTML = todoItems;
        listItems.appendChild(li);
    });
...
function UpdateToDoItems(e) {
    if (e.parentElement.parentElement.querySelector("div").style.textDecoration === "") {
        ...
        addUpdate.setAttribute("src", "img/icon/refresh1.png");
        todoValue.focus();
    }
}
...
function UpdateOnSelectionItems() {
    ...
    addUpdate.setAttribute("src", "img/icon/add.png");
    todoValue.value = "";
...
function DeleteToDoItems(e) {
    ...
    if (confirm(`Do You Want to Delete This ${deleteValue} ?`)) {
        ...
        setLocalStorage();
        setDeleteAlertMessage("Todo item Deleted Successfully!", true);
    }
}
...
function CompletedToDoItems(e) {
    if (e.parentElement.querySelector("div").style.textDecoration === "") {
        ...
        img.src = "img/icon/check-mark.png";
...
function setDeleteAlertMessage(message, isError) {
    todoAlert.removeAttribute("class");
